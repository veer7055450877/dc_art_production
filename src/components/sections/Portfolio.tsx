...
const CustomVideoPlayer = ({ src, poster, hasAudio = true }: { src: string; poster?: string; hasAudio?: boolean }) => {
  ...
  return (
    <div className="relative w-full h-full group bg-black flex items-center justify-center select-none" onMouseMove={handleMouseMove} onClick={togglePlay}>
      <video ref={videoRef} src={src} autoPlay poster={poster} className="max-h-[80vh] max-w-full w-auto h-auto object-contain" onTimeUpdate={handleTimeUpdate} onLoadedMetadata={handleLoadedMetadata} onEnded={() => setIsPlaying(false)} playsInline />
      <AnimatePresence>
        {showNoSoundMsg && (
